...
class ManageTestCase(TestCase):
    ...
    def setUp(self):
        self.test_user = GoodUserFactory.create(
            username='bob', email='bob@localhost')
        self.test_user.set_password("password")
        self.test_user.save()
        self.test_user2 = GoodUserFactory.create(
            username='bleh', email='bleh@localhost')
    ...
    def test_manage_sorting(self):
        ...
        manage_page = self.client.get(
            reverse('manage_org_data') + '?sort=name')
