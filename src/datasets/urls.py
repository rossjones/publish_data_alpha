...
urlpatterns = [
    ...
    # Data file URLs
    url(r'^(?P<dataset_name>[\w-]+)/addfile/(?P<datafile_id>[\w-]+)?$',
    ...
    url(r'^(?P<dataset_name>[\w-]+)/addfile_annually/(?P<datafile_id>[\w-]+)?$',
        login_required(v.edit_addfile_annually),
        name='edit_dataset_addfile_annually'),

    url(r'^(?P<dataset_name>[\w-]+)/deletefile/(?P<datafile_id>[\w-]+)$',
